...
class InputFileConfig:
    ...
    def get_file_col_name(self):
        """
        Return a list with column name of the input files
        """
        return tuple(self.keys())
    
    def get_file_col_type(self):
        return tuple(tupla[0] for tupla in self.cols.values())
    
    def get_table_col_name(self):
        return tuple(tupla[1] for tupla in self.cols.values())
    
    def get_table_col_type(self):
        return tuple(tupla[2] for tupla in self.cols.values())
...
class StationConfig:
    ...
    def get_table_name(self, file_name):
        ...
        given the the indetifier of the files to put in the table.
        """
        return self.db_table_names[file_name]
